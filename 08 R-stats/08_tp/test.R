...
data$장애유형별[data$매우.많음 == max(data$매우.많음)]2
data$장애유형별[data$매우.많음 == min(data$매우.많음)]

# 설문조사의 결과가 4개의 칼럼으로 나누어져 있음
# 각 응답에 대한 가중치를 두어 총합 점수를 계산
# 매우 많음 : 3점 , 약간 받고 있음 : 1점, 별로 받지 못하고 있음 : -1점, 전혀 받지 못하고 있음 : -3점
data$만족도.점수 <- (data$매우.많음 * 3) + data$약간.받고.있음 - data$별로.받지.못하고.있음 - (data$전혀.받지.못하고.있음 * 3)

summary(data$만족도.점수)
# 등급 구분 : 1 사분위 값 이하 = D, 1사분위 값과 2 사분위 값 사이 = C, 2 사분위 값과 3 사분위 값 사이 = B, 3 사분위 값 이상 = A

data$만족도.등급 <- ifelse(data$만족도.점수 >= 6.25, 'A',
                      ifelse(data$만족도.점수 >= -35.70, 'B', 
                             ifelse(data$만족도.점수 >= -66.60, 'C', 'D')))

data$만족여부 <- ifelse(
  (data$만족도.점수 <- data$매우.많음 + data$약간.받고.있음 - data$별로.받지.못하고.있음 - data$전혀.받지.못하고.있음) >= 0,
  '긍정', '부정') 

data <- data[order(data$대분류),]
data <- data[order(data$중분류),]
rownames(data)=NULL

dt_01 <-  mean(data[data$대분류 == '신체.장애',]$매우.많음)
dt_02 <-  mean(data[data$대분류 == '신체.장애',]$약간.받고.있음)
dt_03 <-  mean(data[data$대분류 == '신체.장애',]$별로.받지.못하고.있음)
dt_04 <-  mean(data[data$대분류 == '신체.장애',]$전혀.받지.못하고.있음)

dt_01 + dt_02 + dt_03 + dt_04
dt_01
